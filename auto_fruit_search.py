...
def drive_to_point(waypoint, robot_pose):
    ...
    theta_w = np.arctan2(waypoint[1],waypoint[0])   # angle of waypoint from origin
    theta_turn = theta_w - theta_r                  # angle to be turned by robot
    r = baseline/2
    d_turn = r * theta_turn

    print(theta_r, theta_w, theta_turn)

    # turn towards the waypoint
    turn_time = d_turn / (wheel_vel*scale)          # TO DO: CONVERT WHEEL VEL to CORRECT UNIT (TICKS/s OR RAD/s)
    ...
    d_move = d_waypoint - d_robot                   # in meter

    print(d_robot, d_waypoint, d_move)
    
    # after turning, drive straight to the waypoint
    drive_time = d_move / (wheel_vel*scale)         # TO DO: wheel_vel (tick/s) * scale (m/tick) = meter/s
