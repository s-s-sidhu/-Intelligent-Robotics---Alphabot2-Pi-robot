...
if __name__ == "__main__":
    ...
    start_pose = [0, 0, 0]
    gt_tag, gt_vec, fruit_tag, fruit_vec = get_gt_fruit_vec('generated_map_m5.txt')
    ...
    while True:
            ...
            for fruit_index in range(len(operate.detector.border)):
                ...
                if label_str == fruit_search[i_fruit]:
                    ...
                    if (ymax) < 450:
                        ...
                        if (xmiddle < 200):
                            print("FRUIT AT LEFT!")
                            # B1: TURN LEFT (roughly 22.5 degrees)       ### TODO: TEST
                            velo = ({
                                'velocity': [0, 1],
                                'turn_tick': 15,
                                'time': 0.15
                        ...
                        # fruit at right
                            ...
                            # B2: TURN RIGHT (roughly 22.5 degrees)       ### TODO: TEST
                            velo = ({
                                'velocity': [0, -1],
                                'turn_tick': 15,
                                'time': 0.13
                        ...
                        # B3: GO STRAIGHT after turning (roughly 5 cm)
                        velo = ({
                            'velocity': [1, 0],
                            'tick': 20,
                            'time': 0.5,
        ...
        if not(fruit_found):
            ...
            # try turning for num_turn_thres times
            if (num_turn) < num_turn_thres:
                # PERFORM FRUIT DETECTION
                operate.take_pic()
                operate.command['inference'] = True
                operate.detect_target()
                # C1: TURN RIGHT (roughly lesser than 22.5 degrees)       ### TODO: TEST
                velo = ({
                    'velocity': [0, -1],
                    'turn_tick': 20,
                    'time': 0.1
            ...
            # go straight
                ...
                moving_straight = True
                if (num_straight == 0):
                    # C2: TURN LEFT BACK to normal       ### TODO: TEST
                    velo = ({
                        'velocity': [0, 1],
                        'turn_tick': 20,
                        'time': 0.13
                ...
                # try going straight for num_straight_thres times
                if (num_straight) < num_straight_thres:

                    # C3: GO STRAIGHT after turning (roughly 5 cm)  ### TODO: TEST
                    velo = ({
                        'velocity': [1, 0],
                        'tick': 20,
                        'time': 0.5,
